...
if __name__ == '__main__':
    ...
    ti.init(ti.gpu, device_memory_fraction=0.9, debug=True)
    fluid = Fluid(fluid_ply_prefix, trimesh.creation.box([0.6, 0.8, 0.6]), [1.0, -1.0, -6])
    rigid1 = Rigid(rigids_obj_prefix[0], trimesh.creation.torus(0.2, 0.1), [1.1, 0.4, -5.9], density=500.0, fixed=False)
    rigid2 = Rigid(rigids_obj_prefix[1], trimesh.creation.torus(0.2, 0.1), [1.0, -0.4, -6], density=500.0)
    rigid3 = Rigid(rigids_obj_prefix[2], trimesh.creation.torus(0.2, 0.1), [1.0, 0.0, -5.7], [0.0, 1.0, 0.0], density=500.0, fixed=False)
    scene = Scene([1.0, 2.0, 1.0], [1.0, -0.45, -6], None, [rigid1, rigid2, rigid3], 1.0 / 60)
