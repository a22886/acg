...
from main import has_rigids, has_fluid
import time, tqdm
...
class Scene:
    def __init__(self, dimensions=[1.0, 2.0, 1.0], center=[0.0, 0.0, 0.0], fluid: Fluid = None, rigids: list[Rigid] = [], time_step=1.0 / 60):
    ...
    def start_for_frames(self, frames):
        for i in range(frames):
            if i == 0:
                ...
                for _ in tqdm.tqdm(
                    range(self.subframes),
                    desc=f"Frame {i - 1 :>3} to {i :>3}, start: {time.strftime('%H:%M:%S')}",
                ):
    ...
    def write(self, i):
        if (self.fluid is not None) and has_fluid:
            self.fluid.write(i)
        for j in range(len(self.rigids)):
            if (self.rigids[j] is not None) and has_rigids[j]:
                self.rigids[j].write(i)
